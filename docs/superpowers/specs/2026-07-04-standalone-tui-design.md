...
- `go.mod`: `module github.com/hyperagent/tui`, own `go build` target,
  ...
  `backend` module.
- Rendering/layout/keybinding files (`layout.go`, `render.go`, `view.go`,
  `theme.go`, `markdown.go`, `overlays.go`, `helpview.go`, `detail.go`,
  `signalview.go`, `ideas.go`, `markets.go`) move over unchanged — they
  operate on `Model`'s own fields, not on backend internals.
...
### 4. `backend/src/main.go` simplification
...
- `tui/` module: existing `backend/internal/tui/*_test.go` files that test
  ...
  package gets its own tests against an `httptest.Server` standing in for the
  backend, verifying request shapes and response decoding — no live backend
  needed to test the TUI.
...
- No change to the dashboard or its existing API usage.
- No change to the wire-level behavior of any existing endpoint.
