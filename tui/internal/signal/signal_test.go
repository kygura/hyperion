...
func TestOIPriceQuadrants(t *testing.T) {
	...
	for _, c := range cases {
		s, ok := oiPrice(Inputs{Cur: apiclient.Bar{OIDelta: c.oi, Return: c.ret}})
		if !ok {
...
func TestFundingRegimeCrowdedLongs(t *testing.T) {
	...
	var hist []apiclient.Bar
	for _, f := range noise {
		hist = append(hist, apiclient.Bar{Funding: f})
	}
	s, ok := fundingRegime(Inputs{Cur: apiclient.Bar{Funding: 3.0e-4}, History: hist})
	if !ok {
...
func TestCVDBearishDivergence(t *testing.T) {
	...
	var hist []apiclient.Bar
	for i := 0; i < 6; i++ {
		hist = append(hist, apiclient.Bar{
			Close: 100 + float64(i),
...
func TestMoveSignificanceNormalizesByVol(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		hist = append(hist, apiclient.Bar{Return: r})
	}
	s, ok := moveSignificance(Inputs{Cur: apiclient.Bar{Return: 0.01}, History: hist})
	if !ok {
