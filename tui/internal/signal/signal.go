...
// Inputs is assembled by the caller so apiclient.Bar stays lean. History is the
...
type Inputs struct {
	Cur     apiclient.Bar
	History []apiclient.Bar
	Ctx     apiclient.AssetCtx
...
func lastN(bars []apiclient.Bar, n int) []apiclient.Bar {
	if len(bars) <= n {
		return bars
	}
