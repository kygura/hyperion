...
func fundingRegime(in Inputs) (Signal, bool) {
	fund := in.Cur.Funding
	series := seriesOf(in.History, func(b apiclient.Bar) float64 { return b.Funding })
	if len(series) < 4 {
...
func cvdDivergence(in Inputs) (Signal, bool) {
	...
	cvdRange := seriesRange(seriesOf(bars, func(b apiclient.Bar) float64 { return b.CVD }))
	if cvdRange == 0 {
		return Signal{}, false
	}
...
func moveSignificance(in Inputs) (Signal, bool) {
	ret := in.Cur.Return
	sigma := stddev(seriesOf(in.History, func(b apiclient.Bar) float64 { return b.Return }))
	if sigma <= 0 {
...
func volRegime(in Inputs) (Signal, bool) {
	vols := seriesOf(in.History, func(b apiclient.Bar) float64 { return b.RealizedVol })
	if len(vols) < 6 {
		return Signal{}, false
	}
